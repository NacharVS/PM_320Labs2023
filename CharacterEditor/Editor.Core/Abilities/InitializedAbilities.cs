...
public static class InitializedAbilities
{
    public static Dictionary<int, Ability> Abilities = new Dictionary<int, Ability>()
    {
        ...
        {3, new Ability("Iron fists", 6, x => x.PhysicalDamage += 25)},
        {4, new Ability("Genius brain", 6, x => x.MagicDamage += 18)},
        {5, new Ability("Holy arms", 9, x => x.MagicDamage += 35)},
        {6, new Ability("Double Upgrade Health", 9, x => x.HealthPoints += 35)},
        {7, new Ability("Double Upgrade Physical Defense", 3, x => x.PhysicalDefense += 35)},
        {8, new Ability("Daedra skin", 12, x => x.PhysicalDefense += 55)},
        {9, new Ability("Valkyrie bless", 12, x => x.MagicDamage += 55)}
    };
}