        ...
        switch (atUnit.GetType().Name.ToString())
        {
            ...
            case "Archer":
                if (time % ((Archer)atUnit).attackSpeed == 0)
                {
                    ...
                    try
                    {
                        ReportDamage(atUnit, (int)(((Archer)atUnit).damage -
                            ((Archer)atUnit).damage * (double)((Military)defUnit).armor / 100), defUnit);
                    }

                    catch { ReportDamage(atUnit, ((Archer)atUnit).damage, defUnit); }
                }
                break;
